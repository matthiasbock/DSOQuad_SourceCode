...
void Int2Str(u8 *p, s32 n, uc8 *pUnit, u8 e, u8 Mode)
{
  ...
  p += e; k = p+1;  
  while(n >= Power(10, e)) n /= 10;  // ��ȡ���eλ��Чλ��
  for(j=0; j<m; j++) while(*pUnit++);
  do *k++ = *pUnit; while(*pUnit++); // �����ַ��ַ���  
  while(e--){
    *p-- = '0'+(n%10); n /= 10;
...
*******************************************************************************/
u8 Str2Byte(u8 x,u8 y) // ˫ASCII�ַ�ת1�ֽڶ�������
{
  uc8 Hexcode[17]="0123456789ABCDEF";
  u8 i, Temp=0;
  
  if(x>='a' && x<='z')  x-=32;     // Сд�Ĵ�д
  if(y>='a' && y<='z')  y-=32;     // Сд�Ĵ�д
  for(i=0;i<16;i++){
    if(Hexcode[i]==x)  Temp+=i*16; // ���ַ�תΪ��4λʮ��������ֵ
  }
  for(i=0;i<16;i++){
    if(Hexcode[i]==y)  Temp+=i;    // ���ַ�תΪ��4λʮ��������ֵ
  }
  return Temp;
}
