...
//  0x020,0x020,0x040,0x780,0x000,0x000,0x000,0x000, // # ���Ͻ�
  0x1FC,0x1FC,0x1FC,0x1FC,0x1FC,0x1FC,0x1FC,0x000, // # ����
//  0x000,0x000,0x000,0x00F,0x010,0x020,0x020,0x020, // $ ���½�
  0x070,0x0F8,0x1FC,0x1FC,0x1FC,0x0F8,0x070,0x000, // $ Բ��
  ...
  0x0F8,0x088,0x38E,0x202,0x202,0x202,0x202,0x202};// ~ �յ��ͷ
...
uc16  Row_Base3[201] =     //��ˮƽ���ߺ�ˮƽ���ߵ��л�������
{GRAY ,BLACK,BLACK,BLACK,BLACK,BLACK,BLACK,BLACK,BLACK,BLACK, // 00~10
 BLACK,BLACK,BLACK,BLACK,BLACK,BLACK,BLACK,BLACK,BLACK,BLACK, // 10~20
...
void Print_Clk(u16 x0, u16 y0, u16 Type, u8 Phase)
{
  ...
  __LCD_Set_Block(LCD_X1,LCD_X2,LCD_Y1,LCD_Y2);  //�ָ�ȫ�ߴ細��
}
/*******************************************************************************
 Print_Str: 
*******************************************************************************/
...
{ 
  u8  i, y[8], Dot_Hide[8]; 
  s16 Tmp, m, n ;
  if((Row > MIN_X)&&(Row <= MAX_X)){               // ������ʾ������Ԥ����
    m = (Row - MIN_X-1)* 4;
    n = (Row - MIN_X)  * 4;
    for(i = 0; i < 8; i += 2) {
      Dot_Hide[i] = 0;
      y[i]   = TrackBuff[m + i/2];                  // �˵���ȡ
      y[i+1] = TrackBuff[n + i/2];
      
      if(y[i]   >= Y_BASE+Y_SIZE)  y[i]   = Y_BASE+Y_SIZE-1;      // ���紦��
      else if(y[i]   <= Y_BASE+1)  y[i]   = Y_BASE+1;   
      if(y[i+1] >= Y_BASE+Y_SIZE)  y[i+1] = Y_BASE+Y_SIZE-1;
      else if(y[i+1] <= Y_BASE+1)  y[i+1] = Y_BASE+1;
      
      if(y[i] == y[i+1]){
        if((y[i] == Y_BASE+1)||(y[i] == Y_SIZE-1)) Dot_Hide[i] = 1;  // ��������
        else {
          if(y[i] >= Y_BASE+2)           y[i]   -= 1;              // ˮƽ�߼Ӵ�
...
//------------------------- Draw the Curve data --------------------------------
    if((Row > MIN_X)&&(Row < MAX_X)){             
      if((Dot_Hide[0] == 0)&&(Title[TRACK1][SOURCE].Value != HIDE)){
        if((y[1]-y[0])>5){
          for(i=y[0]; i<=y[1]; ++i) LCD_Buffer2[i] |=Color[TR_1]-0x4200; // ������
        } else {
          for(i=y[0]; i<=y[1]; ++i) LCD_Buffer2[i] |=Color[TR_1];        // ��������
        }
      }
      if((Dot_Hide[2] == 0)&&(Title[TRACK2][SOURCE].Value != HIDE)){
        if((y[3]-y[2])>5){
          for(i=y[2]; i<=y[3]; ++i) LCD_Buffer2[i] |=Color[TR_2]-0x0208; // ������
        } else {   
          for(i=y[2]; i<=y[3]; ++i) LCD_Buffer2[i] |=Color[TR_2];        // ��������
        }
      }
      if((Dot_Hide[4] == 0)&&(Title[TRACK3][SOURCE].Value != HIDE)){
        if((y[5]-y[4])>5){
          for(i=y[4]; i<=y[5]; ++i) LCD_Buffer2[i] |=Color[TR_3]-0x4008; // ������
        } else {
          for(i=y[4]; i<=y[5]; ++i) LCD_Buffer2[i] |=Color[TR_3];        // ��������
        }
      }
      if((Dot_Hide[6] == 0)&&(Title[TRACK4][SOURCE].Value != HIDE)){
        if((y[7]-y[6])>5){
          for(i=y[6]; i<=y[7]; ++i) LCD_Buffer2[i] |=Color[TR_4]-0x0200; // ������
        } else {
          for(i=y[6]; i<=y[7]; ++i) LCD_Buffer2[i] |=Color[TR_4];        // ��������
        }
      }
...
//------------------------- Draw the Curve data --------------------------------
    if((Row > MIN_X)&&(Row < MAX_X)){              
      if((Dot_Hide[0] == 0)&&(Title[TRACK1][SOURCE].Value != HIDE)){
        if((y[1]-y[0])>5){
          for(i=y[0]; i<=y[1]; ++i) LCD_Buffer1[i] |=Color[TR_1]-0x4200; // ������
        } else {
          for(i=y[0]; i<=y[1]; ++i) LCD_Buffer1[i] |=Color[TR_1];        // ��������
        }
      }
      if((Dot_Hide[2] == 0)&&(Title[TRACK2][SOURCE].Value != HIDE)){
        if((y[3]-y[2])>5){
          for(i=y[2]; i<=y[3]; ++i) LCD_Buffer1[i] |=Color[TR_2]-0x0208; // ������
        } else {   
          for(i=y[2]; i<=y[3]; ++i) LCD_Buffer1[i] |=Color[TR_2];        // ��������
        }
      }
      if((Dot_Hide[4] == 0)&&(Title[TRACK3][SOURCE].Value != HIDE)){
        if((y[5]-y[4])>5){
          for(i=y[4]; i<=y[5]; ++i) LCD_Buffer1[i] |=Color[TR_3]-0x4008; // ������
        } else {
          for(i=y[4]; i<=y[5]; ++i) LCD_Buffer1[i] |=Color[TR_3];        // ��������
        }
      }
      if((Dot_Hide[6] == 0)&&(Title[TRACK4][SOURCE].Value != HIDE)){
        if((y[7]-y[6])>5){
          for(i=y[6]; i<=y[7]; ++i) LCD_Buffer1[i] |=Color[TR_4]-0x0200; // ������
        } else {
          for(i=y[6]; i<=y[7]; ++i) LCD_Buffer1[i] |=Color[TR_4];        // ��������
        }
      }
//------------------------- Draw the X Vernie data -----------------------------
      Tmp =(MIN_X + 150)- _X_posi.Value;
